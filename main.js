...
function rollit(ricks = 0) {
    ...
    div.innerHTML = '<video autoplay loop playsinline><source src="' + rollrickBlobURL + '" type="video/mp4"></video>'
...
function randoScroll() {
    var scrollPos = Math.floor(Math.random() * (document.body.scrollHeight + 1));
...
async function onRealLoad() {
    rollrickBlobURL = await fetch(rollrick);
    rollrickBlobURL = await rollrickBlobURL.blob();
    rollrickBlobURL = await URL.createObjectURL(rollrickBlobURL);
